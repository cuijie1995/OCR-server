...
export default {
  ...
  methods: {
    getRequest() {
      ...
      // var url =
      // "http://yun.witsoftware.cn:8890/#/preview?id=ba347ce1-75c2-43a1-b372-709d3ac9502a&name=aaaa";
      var theRequest = new Object();
      // if (url.indexOf("?") != -1) {
      var str = url.substr();
      // console.log(str.split("&"))
      // str = str.split("&")
      str = str.split("?") || str.split("&");
      console.log(str);
      ...
      self.theRequestId = theRequest.id;

      if (theRequest.id) {
        localStorage.setItem("urlId", theRequest.id);
        localStorage.setItem("pdfOcrId", theRequest.id);
    ...
    clickRefresh() {
      ...
      // self.$router.push({
      //   path: "/TableIndex",
      //   query: { id: localStorage.getItem("pdfOcrId") },
      // });
    ...
    handleChange(file, fileList) {
      ...
      UploadIForma(formData).then((res) => {
        self.pdfId = res.result.enclosures[0].id;
        localStorage.setItem("pdfOcrId", res.result.enclosures[0].id);
        localStorage.setItem("urlId", res.result.enclosures[0].id);
